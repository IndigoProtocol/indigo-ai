...
Get the current chain synchronization status for the Indigo Protocol indexer.

**Parameters:** None

**Returns:** Sync status object:
- `currentBlock` — Latest synced block height
- `chainTip` — Current chain tip block height
- `syncPercentage` — Sync progress as percentage
- `lastUpdated` — Timestamp of last sync update
